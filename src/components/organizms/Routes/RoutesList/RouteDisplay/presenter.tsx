...
const RouteDisplay: VFC<Props> = ({
  ...
  <div
    style={{ width: isWide ? '415px' : '310px' }}
    className="border-2 hover:bg-gray-50 rounded-md"
  >
    <RouteSummary route={route} tlDic={tlDic} />
    <div className="mb-2 ml-2">
      <div className="mt-2 text-center text-white rounded-full w-14 bg-turquoise-500">1凸目</div>
      <div>
        <TLThumbnail
      ...
      {calculatedAttackNumConditions >= 2 && (
        <>
          <div className="mt-2 text-center text-white rounded-full w-14 bg-turquoise-500">
            2凸目
          </div>
          <div>
            <TLThumbnail
